...
1. **Python Format**
   ...
   - Command: `make format`
   - test_name: "python_format_check"
   - test_purpose: "Validates Python code formatting, ensuring consistent indentation, line length, and style"
...
3. **Python Type Checking**
   ...
   - test_name: "python_type_checking"
   - test_purpose: "Validates Python code type correctness, ensuring type annotations are consistent and correct"
...
5. **SonarQube Scan**
   - Preparation Command: None
   - Command: `sonar-scanner`. Then fetch the results from the SonarQube API. Use the `SONAR_TOKEN` environment variable to authenticate.
   - test_name: "sonar_scan"
   - test_purpose: "Validates SonarQube scan results, ensuring the application is in a healthy state"
...
[
  {
    ...
    "test_purpose": "Validates Python code quality, identifies unused imports, style violations, and potential bugs",
    "error": "ANN201 Missing return type annotation for public function `get_metric_config`\n --> app/domain/ports/metric_config_repository.py:6:15"
  },
  {
    "test_name": "python_format",
    "passed": true,
    "execution_command": "make format",
    "test_purpose": "Validates Python code formatting, ensuring consistent style and readability"
  }
]
```
